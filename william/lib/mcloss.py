...
class MCLoss(nn.Module):
    def __init__(
        self, num_classes=200, cnums=[10, 11], cgroups=[152, 48], p=0.4, lambda_=10
    ):
        super().__init__()
        if isinstance(cnums, int):
            ...
            cnums = list(cnums)
        assert isinstance(cnums, list), print(
            "Error: cnums should be int or a list of int, not {}".format(type(cnums))
        )
    ...
    def forward(self, feat, targets):
        ...
        for i in range(1, len(sp)):
            feat_group.append(
                F.softmax(feature[:, sp[i - 1] : sp[i]].view(n, -1, h * w), dim=2).view(
                    n, -1, h, w
                )
            )  # Softmax
        ...
        for i in range(len(self.cnums)):
            ...
            l_div = l_div + (
                1.0 - torch.mean(torch.sum(features, dim=2)) / (self.cnums[i] * 1.0)
            )
    ...
    def _gen_mask(self, cnums, cgroups, p):
        ...
        for i in range(len(cnums)):
            ...
            for j in range(cgroups[i]):
                drop_idx.append(
                    np.random.choice(np.arange(cnums[i]), size=drop_num, replace=False)
                    + j * cnums[i]
                )
...
if __name__ == "__main__":
    ...
    # mcloss = MCLoss(p=0)
    # targets = torch.randn(2, 1000)
    # feat = torch.randn((2, 3000, 14, 14))
    # loss = mcloss(feat, targets)
    # print(loss)

    mcloss = MCLoss(
        num_classes=200, cnums=[10, 11], cgroups=[152, 48], p=0.4, lambda_=10
    )
